 ...
 *   verify            Verify SSOT registries, checksums, and glossary schema
 *   help              Show help
 ...
 *   add-term          Add a term to docs/context/knowledge/glossary.json
 *   remove-term       Remove a term from docs/context/knowledge/glossary.json
...
Commands:
  ...
  add-term
    ...
    --definition <string>       Definition (required)
    --scope <string>            Scope: "global" or a module name (optional)
    --aliases <csv>             Comma-separated aliases (optional)
    --see-also <csv>            Comma-separated related terms (optional)
    --repo-root <path>          Repo root (default: cwd)
    Add a term to docs/context/knowledge/glossary.json.

  remove-term
    --term <string>             Term name to remove (required)
    --repo-root <path>          Repo root (default: cwd)
    Remove a term from docs/context/knowledge/glossary.json.
...
function cmdBuild(repoRoot, opts) {
  ...
  if (existing) {
    const stableNew = JSON.stringify({ ...derived, updatedAt: '__STABLE__' });
    const stableOld = JSON.stringify({ ...existing, updatedAt: '__STABLE__' });
    if (stableNew === stableOld) {
      derived.updatedAt = existing.updatedAt;
    }
  }
...
async function cmdVerify(repoRoot, strict) {
  ...
  // Derived registry freshness check
  const derivedPath = getDerivedRegistryPath(repoRoot);
  if (fs.existsSync(derivedPath)) {
    const onDisk = readJsonOrNull(derivedPath);
    const onDiskIds = new Set((onDisk?.artifacts || []).map(a => a.id || `${a.moduleId}:${a.artifactId}`));
    const expectedIds = new Set();
    for (const p of paths) {
      const l = loadModuleRegistry(p);
      if (!l.ok) continue;
      for (const a of l.registry.artifacts || []) {
        expectedIds.add(`${l.registry.moduleId}:${a.artifactId ?? a.id}`);
      }
    }
    for (const eid of expectedIds) {
      if (!onDiskIds.has(eid)) {
        warnings.push(`Derived registry missing artifact: ${eid} (run: ctl-context build)`);
      }
    }
    for (const did of onDiskIds) {
      if (!expectedIds.has(did)) {
        warnings.push(`Derived registry has stale artifact: ${did} (run: ctl-context build)`);
      }
    }
  } else {
    warnings.push(`Derived registry not found: ${path.relative(repoRoot, derivedPath)} (run: ctl-context build)`);
  }
...
function getGlossaryPath(repoRoot) {
  return path.join(getContextDir(repoRoot), 'knowledge', 'glossary.json');
}

function getGlossarySchemaPath(repoRoot) {
  return path.join(getContextDir(repoRoot), 'knowledge', 'glossary.schema.json');
}

function loadGlossary(repoRoot) {
  const gp = getGlossaryPath(repoRoot);
  const data = readJsonOrNull(gp);
  if (!data) return null;
  return {
...
async function validateGlossarySchema(repoRoot) {
  ...
  const raw = readJsonOrNull(glossaryPath);
  ...
  const schema = readJsonOrNull(schemaPath);
  if (!schema) {
